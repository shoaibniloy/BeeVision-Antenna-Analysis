...
_PIPELINE_DEBUG = False  # Flip to True only when debugging the antenna pipeline
...
class ResearchVisualizationWidget(QWidget):
    ...
    def _update_panel_image(self, panel_id, img_array):
        ...
        # FastTransformation is sufficient for ~15 fps live panels;
        # SmoothTransformation is reserved for the zoom dialog (static view).
        pixmap = QPixmap.fromImage(q_img)
        scaled_pixmap = pixmap.scaled(
            image_label.size(),
            Qt.AspectRatioMode.KeepAspectRatio,
...
class AntennaProcessingPipeline:
    ...
    def process_antenna(self, blob_mask, head_point, body_center,
        ...
        if blob_mask is None or blob_mask.size == 0:
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Blob mask is None or empty")
            return results
        
        try:
            # Stage H: Original antenna blob
            results['original_blob'] = blob_mask.copy()
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Stage H: Original blob size = {blob_mask.shape}")
            ...
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Stage I: After closing")
            ...
            for lbl in range(1, num_labels):       # 0 = background
                ...
                if elong < MIN_ELONGATION:
                    if _PIPELINE_DEBUG: print(f"[ANTENNA] Component {lbl}: area={area}, "
            ...
            if not scored:
                if _PIPELINE_DEBUG: print("[ANTENNA] Stage I.5: No elongated components found — "
            ...
            if not scored:
                if _PIPELINE_DEBUG: print("[ANTENNA] Stage I.5: No valid components found at all")
                return results

            # Sort descending by score, take best 2
            scored.sort(key=lambda x: x[0], reverse=True)
            top2 = scored[:2]
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Stage I.5: Keeping {len(top2)} antenna component(s): "
            ...
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Stage J (raw): Skeleton has {skeleton_pixels_raw} pixels")

            if skeleton_pixels_raw == 0:
                if _PIPELINE_DEBUG: print("[ANTENNA] No skeleton pixels after first pass - aborting")
                return results
            ...
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Stage J.5 (clean): Skeleton has {skeleton_pixels} px "
            ...
            if skeleton_pixels == 0:
                ...
                if _PIPELINE_DEBUG: print("[ANTENNA] Re-skeletonize collapsed skeleton — using raw")
            ...
            def _process_side_skeleton(side_skel, side_label):
                ...
                if np.count_nonzero(side_skel) < 2:
                    if _PIPELINE_DEBUG: print(f"[ANTENNA] {side_label}: <2 skeleton pixels — skipped")
                    return
                path = self.find_longest_antenna_path(
                    side_skel, head_point, body_center)
                if len(path) < 2:
                    if _PIPELINE_DEBUG: print(f"[ANTENNA] {side_label}: path too short — skipped")
                    return
                ...
                if _PIPELINE_DEBUG: print(f"[ANTENNA] {side_label}: path={len(path)} pts, "
            ...
            if roi_guided:
                ...
                if _PIPELINE_DEBUG: print("[ANTENNA] Stage K+L: Using ROI-guided skeleton split")
                ...
                if _PIPELINE_DEBUG: print("[ANTENNA] Stage K+L: Using component-based skeleton split")
                for _, lbl, _, _ in top2:
            ...
            def _synth_path(direction_x, direction_y, label):
                ...
                for i in range(SYNTH_POINTS):
                    ...
                    pts.append((px, py))
                if _PIPELINE_DEBUG: print(f"[ANTENNA] {label}: SYNTHETIC fallback path "
            ...
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Final: {len(results['bfs_paths'])} antenna path(s) found")
            
        except Exception as e:
            if _PIPELINE_DEBUG: print(f"[ANTENNA] Error in processing pipeline: {e}")
            import traceback
            traceback.print_exc()
    ...
    def find_longest_antenna_path(self, skeleton, head_point, body_center):
        ...
        if skeleton is None or skeleton.size == 0:
            if _PIPELINE_DEBUG: print("[ANTENNA_PATH] Skeleton is None or empty")
            return []
        ...
        # ── Vectorised spur pruning — no Python loops, ~100× faster ──────────────
        # Each pass strips one layer from every 1-neighbour endpoint pixel.
        # SPUR_MAX_LEN passes therefore prune every branch shorter than that many
        # pixels — produces identical results to the old per-pixel tracer.
        _sp_k = np.ones((3, 3), dtype=np.float32); _sp_k[1, 1] = 0.0
        for _prune_round in range(SPUR_MAX_LEN):
            if np.count_nonzero(pruned) < 3:
                break
            _sp_bin = (pruned > 0).astype(np.uint8)
            _sp_nb  = cv2.filter2D(_sp_bin.astype(np.float32), -1, _sp_k,
                                   borderType=cv2.BORDER_CONSTANT).astype(np.uint8)
            _sp_ep  = (_sp_bin > 0) & (_sp_nb == 1)   # endpoint mask
            if not _sp_ep.any():
                break
            pruned[_sp_ep] = 0
        ...
        if pruned_count >= 3:
            work_skel = pruned
            if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] Spur pruning: {np.count_nonzero(skeleton)} → "
                  f"{pruned_count} px ({np.count_nonzero(skeleton) - pruned_count} pruned)")
        else:
            work_skel = skeleton
            if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] Spur pruning removed too much — using original skeleton")

        skeleton_yx = np.argwhere(work_skel > 0)
        if len(skeleton_yx) == 0:
            if _PIPELINE_DEBUG: print("[ANTENNA_PATH] No skeleton pixels found")
            return []

        if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] {len(skeleton_yx)} skeleton pixels for BFS")
        ...
        if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] BASE = {base_pt}  "
        ...
        if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] BFS reached {bfs_count}/{skel_count} skeleton pixels "
        ...
        if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] {len(endpoints)} endpoint candidates")
        ...
        if not tip_candidates:
            if _PIPELINE_DEBUG: print("[ANTENNA_PATH] No tip candidates — returning base-only path")
            return [base_pt]
        ...
        if tip_pt is None:
            if _PIPELINE_DEBUG: print("[ANTENNA_PATH] Could not determine tip point")
            return []

        if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] TIP  = {tip_pt}  "
        ...
        if _PIPELINE_DEBUG: print(f"[ANTENNA_PATH] Final path: {len(path)} points  "
...
class MorphologicalPipelineProcessor:
    ...
    def compute_all_stages(self, frame_gray, frame_id=0, all_bee_data=None, antenna_pipeline=None, worker_instance=None, visualize=True):
        """
        ...
        # Normalisation only needed to generate display panels
        if visualize:
            ...
            tophat_vertical = tophat_horizontal = tophat_diagonal = combined = None
        
        # Panel BGR conversion + ROI triangles — only when rendering for UI
        if visualize:
            ...
            tophat_vertical_bgr = tophat_horizontal_bgr = tophat_diagonal_bgr = combined_bgr = None
        ...
        if visualize:
            ...
            final_binary_bgr = None
        
        # Stats (only compute panel stats when rendering)
        ...
        if visualize:
            ...
            white_pixel_count = v_max = h_max = d_max = 0
        
        # =====================================================================
        # PANEL G: Annotated Opening — viz only
        # =====================================================================
        if visualize:
            annotated_opening = cv2.cvtColor(final_binary, cv2.COLOR_GRAY2BGR)
            if all_bee_data is not None and len(all_bee_data) > 0 and worker_instance is not None:
                annotated_opening = self._draw_roi_triangles_on_frame(annotated_opening, all_bee_data, worker_instance)
        else:
            annotated_opening = None
        
        # =====================================================================
        # ANTENNA PIPELINE — always runs for keypoint writeback;
        # generate_viz=False skips canvas allocation when tab is hidden.
        # =====================================================================
        antenna_stages = self._generate_antenna_processing_stages(
            final_binary, frame_gray, all_bee_data, antenna_pipeline,
            worker_instance=worker_instance, generate_viz=visualize
        )
        
        # When not rendering we have done all functional work (keypoint writeback);
        # return None so the UI slot is never called.
        if not visualize:
            return None
    ...
    def _generate_antenna_processing_stages(self, final_binary, frame_gray, all_bee_data, antenna_pipeline,
                                              worker_instance=None, generate_viz=True):
        """
        ...
        # Full-frame canvases — only allocate when UI needs them
        if generate_viz:
            _binary_bgr = cv2.cvtColor(final_binary, cv2.COLOR_GRAY2BGR)
            _gray_bgr   = cv2.cvtColor(frame_gray,   cv2.COLOR_GRAY2BGR)
            stages = {
                'antenna_original_blob':   _binary_bgr.copy(),
                'antenna_after_closing':   _binary_bgr.copy(),
                'antenna_skeleton':        _binary_bgr.copy(),
                'antenna_endpoints':       _gray_bgr.copy(),
                'antenna_bfs_path':        _gray_bgr.copy(),
                'antenna_final_keypoints': _gray_bgr.copy(),
            }
        else:
            stages = {}  # no canvas allocation in functional-only mode
            ...
            for bee_id, (keypoints, box) in all_bee_data.items():
                ...
                if worker_instance is not None and hasattr(worker_instance, 'get_angled_roi_lines'):
                    try:
                        ...
                        if _PIPELINE_DEBUG: print(f"[LAYER1] ROI mask build failed: {_roi_err}")
                ...
                if generate_viz and antenna_results['original_blob'] is not None:
                ...
                if generate_viz and antenna_results['after_closing'] is not None:
                ...
                # ── J: Skeleton overlay ──
                if generate_viz and antenna_results['skeleton'] is not None:
                ...
                for ant_i, path in (enumerate(antenna_results['bfs_paths']) if generate_viz else ()):
                    if len(path) < 2:
                        continue
                    n_pts = len(path)
                    lbl   = str(ant_i + 1)
                ...
                for ant_i, (bp, jp, tp) in (enumerate(zip(
                        ...
                        antenna_results['tip_points'])) if generate_viz else ()):
                    lbl   = str(ant_i + 1)
                    base_g  = to_global(bp)
...
class FourRegionAntennaTracker:
    ...
    def __init__(self):
        ...
        self.history = defaultdict(lambda: {
            'PROTHORAX': deque(maxlen=self.window_frames),
            'MESOTHORAX': deque(maxlen=self.window_frames),
            'METATHORAX': deque(maxlen=self.window_frames),
            'ABDOMEN':    deque(maxlen=self.window_frames),
        })
        ...
        for i, bee_a_id in enumerate(bee_ids):
            for bee_b_id in bee_ids[i+1:]:
                ...
                for region, data in region_data.items():
                    ...
                    # maxlen on the deque auto-evicts oldest entry — O(1), no pop(0)
...
class IndependentAntennaTracker:
    ...
    def __init__(self):
        ...
        self.antenna_history = defaultdict(lambda: deque(maxlen=self.max_history))  # auto-bounded O(1)
...
class InferenceWorker(QObject):
    ...
    def __init__(self, model_path, video_path, thresholds=None, input_mode='video'):
        ...
        # ── Performance gate: skip expensive research panels when tab is hidden ──
        self.research_viz_enabled = True   # toggled by GUI tab-change signal
        self.active_viz_tab       = 0
    ...
    def run(self):
            try:
                ...
                while self._is_running:
                    ...
                    # ── BFS pipeline every 2 frames: always refines kp[5..8] via
                    # temporal tracker writeback.  Panel image generation is skipped
                    # (visualize=False) while the research tab is hidden, saving the
                    # cost of 13 full-frame BGR conversions and canvas allocations.
                    if self.frames_processed % 2 == 0:
                        _viz = getattr(self, 'research_viz_enabled', True)
                        research_data = self.research_processor.compute_all_stages(
                            frame_gray, frame_id, all_bee_data, self.antenna_pipeline,
                            worker_instance=self, visualize=_viz
                        )
                    ...
                    if _PIPELINE_DEBUG: print(f"[TROPHALLAXIS] Frame {frame_id}: Pending={len(self.trophallaxis_detector.pending_events)}, Completed={len(self.trophallaxis_detector.completed_events)}")
                    ...
                    if self.frames_processed % 10 == 0:
                        ...
                        if _PIPELINE_DEBUG: print(f"[METRICS EMIT] Frame {self.frames_processed}: Emitting metrics update")
...
class KeypointViewerGUI(QMainWindow):
    ...
    def _start(self):
        ...
        self.thread.start()
        # Toggle research-viz pipeline based on which tab is visible
        self.tab_widget.currentChanged.connect(self._on_viz_tab_changed)
        self.worker.research_viz_enabled = (self.tab_widget.currentIndex() == 8)
    
    def _on_viz_tab_changed(self, index):
        """Enable/disable the research-viz pipeline as the user switches tabs."""
        if hasattr(self, 'worker') and self.worker is not None:
            self.worker.research_viz_enabled = (index == 8)
            self.worker.active_viz_tab       = index
    
    def _stop(self):
        if self.worker:
            self.worker.stop()
        # Disconnect tab-change signal before worker is destroyed
        try:
            self.tab_widget.currentChanged.disconnect(self._on_viz_tab_changed)
        except Exception:
            pass
        self.btn_apply_thresholds.setEnabled(False)
        self.thread.quit()
